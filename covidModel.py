...
def dummy_symptomatic(t, beta_I, k, c):
    result = odeint(ODEmodel, y0, t, (beta_I, k, c))[:,4]
    return result
...
#general form to get the optimal B_I via curve fitting
def get_optimal_bI(k, c):
    custom_resid = lambda t, bI: dummy_symptomatic(t, bI, k, c) #have to use this to fix k and c so that they're not part of the curve fitting function
    x = curve_fit(custom_resid, t, conf_incidence, p0=(.6,), maxfev=5000)
    return x[0]
